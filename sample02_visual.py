...
def visual_train(history, epochs):
    ...
    # plt.plot(np.arange(epochs), history.history['acc'], label="train-acc")  # 训练模型的准确率
    # plt.legend()
    ...
    # plt.plot(np.arange(epochs), history.history['val_acc'], label="val-acc")  # 模型验证的损失率
    # plt.legend()
    # values = range(0, 30, 5)  # 等间距绘制
    ...
    # plt.show()
    plt.savefig("data/model/visual_train_batch01.png", format="png")
